...
## 9. 今回の修正後の確認（ゲームオーバー・ステージクリア）

| 項目 | 確認内容 |
|------|----------|
| ゲームオーバー時「このステージから再挑戦」 | SPACE/START で retryCurrentStage() が呼ばれ、同じステージのナレーションから再開すること。スコア・解放スキルは維持。 |
| ステージクリア演出 | ボス撃破後は弾を発射しない（canShoot: false）。約0.75秒後からカラスが高速で右へダッシュし、フェード後に次ステージへ。 |
| エンディングBGM | config のパスは `audio/ending.mp3`。実ファイルが `endding.mp3` の場合は `ending.mp3` にリネームすること。 |
| スキル6（ラスボス解放） | 発動で12発扇状ビーム＋フラッシュ＋シェイク。iOS でも軽量に動作すること。 |
